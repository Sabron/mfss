...
def get_ajax(request):
    try:
        if request.method == "POST":
            sensor_list = DcsSensor.objects.filter(active=True).all().order_by('name')
            m_sensor = []
...
def MainIndexDefault(request):
     if request.method == "GET":
        ...
        for sensor in sensor_list:
            ...
            sensor_dict.update(value=sensor.value / ratio)
            ...
            color = 'bg-success'
            if sensor.critical_type == 'max':
                if sensor.value / ratio >= sensor.critical_value:
                    color = 'bg-danger'
            else:
                if sensor.value / ratio <= sensor.critical_value:
                    color = 'bg-danger'
        ...
        return render(request, 'dcs_main.html',context) 
...
def SensorList(request):
    try:
        if request.method == "GET":
            ...
            return render(request, 'dcs_sensor_list.html',context) 
    except Exception as err:
        logging.error(traceback.format_exc())
    ...
    try:
        if request.method == "GET":
            ...
            if param['sensor_type'] == 'sec':
                strftime = "%H:%M:%S"
                sensor_list = DcsIndicators.objects.filter(sensor=sensor).order_by('-date_time','-value')[:100]
                ...
                strftime = "%H:%M"
                sensor_list = DcsIndicators.objects.filter(sensor=sensor).order_by('-date_time','-value')[:3600]
                #sensor_list = AcsIndicators.objects.filter(sensor=sensor).annotate(
                ...
                strftime = "%H:00"
                sensor_list = DcsIndicators.objects.filter(sensor=sensor).order_by('-date_time','-value')[:120000]
                #sensor_list = AcsIndicators.objects.filter(sensor=sensor).annotate(
