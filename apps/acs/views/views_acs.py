    ...
    try:
        if request.method == "GET":
            ...
            if param['sensor_type'] == 'sec':
                ...
                sensor_list = AcsIndicators.objects.filter(sensor=sensor).order_by('-date_value').annotate(date_value=TruncHour('date_time')).annotate(zn_value =Max('value')).values('date_value','zn_value', 'value', 'sensor__ratio').distinct()[:30]
            m_sensor = []
            for sensor in sensor_list:
