    ...
    try:
        if request.method == "GET":
            ...
            if param['sensor_type'] == 'sec':
                sensor_list = AcsIndicators.objects.filter(sensor=sensor).annotate(date_value=TruncSecond('date_time')).values('date_time', 'date_value', 'value', 'sensor__ratio').order_by('-date_value').distinct('date_value')[:30]
                ...
                sensor_list = AcsIndicators.objects.filter(sensor=sensor).annotate(date_value=TruncHour('date_time')).values('date_time','date_value', 'value', 'sensor__ratio').order_by('-date_value').distinct('date_value').aggregate(Max('value'))[:30]
            m_sensor = []
            for sensor in sensor_list:
