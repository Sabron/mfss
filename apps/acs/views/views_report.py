...
def report_error(request):
    try:
        if request.method == "GET":
            ...
            return render(request, 'report/repor_error.html',context) 
        if request.method == "POST":
            dict_param=request.POST.dict()
...
def report_value(request):
    try:
        if request.method == "GET":
            ...
            return render(request, 'report/repor_value.html',context) 
        if request.method == "POST":
            dict_param=request.POST.dict()
            if dict_param['metod']=='create':
...
def report_list(request):
    try:
        ...
        return render(request, 'acs_report_list.html',context) 
    except Exception as err:
        logging.error(traceback.format_exc())
